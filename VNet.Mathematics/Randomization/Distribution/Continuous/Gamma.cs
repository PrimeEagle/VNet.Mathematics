...
namespace VNet.Mathematics.Randomization.Distribution.Continuous
{
    public class Gamma : RandomDistributionBase, IGammaDistributionAlgorithm
    {
        ...
        public Gamma(double shape, double scale) : base()
        {
        ...
        public Gamma(IRandomGenerationAlgorithm randomGenerator) : base(randomGenerator)
        {
        }

        public Gamma(IRandomGenerationAlgorithm randomGenerator, double shape, double scale) : base(randomGenerator)
        {
            if (shape < 0d) throw new ArgumentOutOfRangeException(nameof(shape), "Must be a positive number.");
            if (scale < 0d) throw new ArgumentOutOfRangeException(nameof(scale), "Must be a positive number.");
